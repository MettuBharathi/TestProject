...
public class CReporterWeb {
	...
	public static Map<BrowserContext, CReporterWeb> passcount = new HashMap<>();
	private String[] package_testname;
	private Integer[] passCont;
	private String[] failCont;
	public List<Integer> iPassCount = new ArrayList<Integer>();
	public List<Integer> iFailCount = new ArrayList<Integer>();
	Map<String, Integer> passCounter = new HashMap<>();
	Map<String, Integer> failCounter = new HashMap<>();
...
// TODO Auto-generated catch block
	...
	public void createHtmlSummaryReport(String Url, boolean append)
		...
		try {
			...
			writer.write("<th class='pass' >#Pass</th> ");
			writer.write("<th class='fail'>#Fail</th> ");
			...
			/*Map<Integer,Integer> testStepPassCount=TestResult.mapBrowserContextTestCasePassValue.get(this.browserContext);
			Map<String,String> testStepFailCount=TestResult.mapBrowserContextTestCaseFailValue.get(this.browserContext);*/

			Map<String,String> p =TestResult.PassNumb.get(this.browserContext);

			Iterator<Entry<String, String>> iterator1 = testCaseRef.entrySet()
					.iterator();
			/*Iterator<Entry<Integer,Integer>> iterator2=testStepPassCount.entrySet().iterator();
			Iterator<Entry<String,String>> iterator3=testStepFailCount.entrySet().iterator();*/
			int serialNo = 1;
			writer.write("<tbody> ");
			//for(int i=1;i<=testCaseRef.size();i++) {
			while (iterator1.hasNext()) {

				int passcount = TestResult.PassNum.get(this.browserContext);
				int failcount = TestResult.FailNum.get(this.browserContext);

				Map.Entry<String, String> mapEntry1 = (Map.Entry<String, String>) iterator1
						.next();
				/*Map.Entry<Integer, Integer> mapEntry2 = (Map.Entry<Integer, Integer>) iterator2
						.next();*/
				/*Map.Entry<String, String> mapEntry3 = (Map.Entry<String, String>) iterator3
						.next();*/
				/*	System.out.println(mapEntry3.getKey());*/
				...
				this.package_testname = mapEntry1.getKey().toString()
						.split(":");
				/*this.passCont[0] = mapEntry2.getKey();
				this.passCont[1] = mapEntry2.getKey();
				this.passCont[1] = mapEntry2.getValue();*/
				...
				if (testCaseExecutionStatus
					...
					writer.write("<td class='justified'><a href='"
							...
							+ "</a></td>");
					System.out.println("Summary Test case" + package_testname[1]);
				} else {
					writer.write("<td class='justified'><a href='"
							+ this.package_testname[1]
							+ "_Results"
				...
				if (TestResult.mapBrowserContextTestCasePassValue != null) {
					Map<Integer, Integer> mapTestDescriptionPass = TestResult.mapBrowserContextTestCasePassValue
							.get(this.browserContext);
					if (mapTestDescriptionPass != null) {
						passCount = mapTestDescriptionPass
								.get(this.passCounter.get(TestResult.strTestName.get(this.browserContext)));
					}
				}
				Integer failCount = null;
				if (TestResult.mapBrowserContextTestCaseFailValue != null) {
					Map<Integer, Integer> mapTestDescriptionFail = TestResult.mapBrowserContextTestCasePassValue
							.get(this.browserContext);
					if (mapTestDescriptionFail != null) {
						failCount = mapTestDescriptionFail
								.get(this.failCounter.get(TestResult.strTestName.get(this.browserContext)));
					}
				}

				System.out.println("Summary Test case" + package_testname[1]);
				System.out.println("Summary Test case " + testCaseRef.size());
				/*System.out.println("Summary Test case Pass Count 0 " + iPassCount.get(0));
				System.out.println("Summary Test case Fail Count 0 " + iFailCount.get(0));
				System.out.println("Summary Test case Pass Count 1 " + iPassCount.get(1));
				System.out.println("Summary Test case Fail Count 1 " + iFailCount.get(1));*/
				writer.write("<td class='justified'>" + localTestDescription
						+ "</td>");
				System.out.println("Summary Report " + localTestDescription);
				writer.write("<td class='pass' >" + passCounter.get(package_testname[1])
						+ "</td>");

				writer.write("<td class='fail'>" + failCounter.get(package_testname[1])
						+ "</td>");
	 ...
	 * @throws Exception
	 */
	private void onSuccess(String strStepName, String strStepDes,String input)
			throws Exception {
		...
		try {
			...
			String strTcTestData=TestResult.tc_testdata.get(this.browserContext);
			...
			System.out.println("PPPPPPPPPPPPPPPPPPPP"+passNumValue);
	...
	private void onFailure(String strStepName, String strStepDes,String input,
		...
		try {
			...
			System.out.println("FFFFFFFFFFFFFFFFFFFFFFFFFF"+failNumValue);
			if (stepNumValue != null) {
			...
			writer.write("<td><small>" + CReporterWeb.getTime() + "</small></td> ");
			writer.write("</tr> ");
			writer.close();
			String strPackageName = TestResult.packageName
					.get(this.browserContext);
	...
	private void onWarning(String strStepName, String strStepDes,String... input) {
		...
		try {
			...
			writer.write("<td class='justified'>" + strStepName + "</td>");
			writer.write("<td class='justified'>" + strStepDes + "</td> ");
			writer.write("<td class='justified'>" + "" + "</td> ");
			...
			//Store the step details in xml
			ReportXML.captureTestStepDetails(TestResult.stepNum.get(CReporterWeb.browserContext), strStepName, strStepDes, "NA",
					strFailTime, "WARNING");
	...
	public void closeDetailedReport() throws IOException {
		...
		Writer writer = null;
		int passcount=TestResult.PassNum.get(this.browserContext);
		iPassCount.add(passcount);
		int failcount=TestResult.FailNum.get(this.browserContext);
		iFailCount.add(failcount);

		passCounter.put(TestResult.strTestName.get(this.browserContext), passcount);
		failCounter.put(TestResult.strTestName.get(this.browserContext), failcount);
/*
Chandu Testing Started
 */
		String pValue=Integer.toString(passcount) ;
		String fValue=Integer.toString(failcount) ;
/*
Pass Mapping
 */
		Map<Integer, Integer> mapTestCasePassValue = TestResult.mapBrowserContextTestCasePassValue
				.get(this.browserContext);
		if (mapTestCasePassValue == null) {
			mapTestCasePassValue = new HashMap<Integer, Integer>();
		}

		mapTestCasePassValue.put(passcount, 0);
		TestResult.mapBrowserContextTestCasePassValue.put(this.browserContext,
				mapTestCasePassValue);
/*
Fail Mapping
 */
		Map<String, String> mapTestCaseFailValue = TestResult.mapBrowserContextTestCaseFailValue
				.get(this.browserContext);
		if (mapTestCaseFailValue == null) {
			mapTestCaseFailValue = new HashMap<String, String>();
		}

		mapTestCasePassValue.put(failcount, 1);
		TestResult.mapBrowserContextTestCaseFailValue.put(this.browserContext,
				mapTestCaseFailValue);
/*
Chandu Testing Ended
 */
	...
	public synchronized void closeSummaryReport() throws IOException {
		...
		try {
			...
			e.printStackTrace();
			LOG.info("===========================Close Summary Report =======Failed to close Summary Report=======" +e.getMessage());
		}
	...
	public void SuccessReport(String strStepName, String strStepDes,String input)
		...
		switch (ReporterConstants.REPORT_FORMAT.toLowerCase()) {
			...
			case "html":
				...
				this.onSuccess(strStepName, strStepDes,input);
				break;
			default:
				/* take screen shot if Screenshot is required for passed test cases */
				if (ReporterConstants.BOOLEAN_ONSUCCESS_SCREENSHOT == true) {
	...
	public void failureReport(String strStepName, String strStepDes,String input,
		...
		switch (ReporterConstants.REPORT_FORMAT.toLowerCase()) {
			...
			case "html":
				...
				this.onFailure(strStepName, reportDescription,input,fileName);
			...
			default:
				...
				this.onFailure(strStepName, strStepDes,input,fileName + ".jpeg");
				// New Screenshot code Starts//
				// if(!screenShotSource.equalsIgnoreCase("Selenium"))
				// screenShotSource = "Selenium";
				// New Screenshot code ends//
	...
	public void warningReport(String strStepName, String strStepDes,String...input)
			throws Throwable {
		...
		switch (ReporterConstants.REPORT_FORMAT.toLowerCase()) {
			...
			case "html":
				...
				this.onWarning(strStepName, strStepDes,input);
				break;
	...
	public void initTestCaseDescription(String testCaseDescription) {
		if (testCaseDescription != null) {
			...
			LOG.info("TC descr: " + testCaseDescription);
			mapTestDescription.put(TestResult.tc_name.get(this.browserContext),
					testCaseDescription);
			TestResult.testDescription.put(this.browserContext,
					mapTestDescription);
			System.out.println("in initTestCaseDescription() test case descr : " +
					TestResult.testDescription.get(this.getBrowserContext()).get(TestResult.tc_name));
		}
	}

	public void initPassCountUpdated(String passCount) {
		if (passCount != null) {
			Map<String, String> mapTestDescription = TestResult.PassNumb
					.get(this.getBrowserContext());
			if (mapTestDescription == null) {
				mapTestDescription = new HashMap<String, String>();
