...
public class MIC02_MIGlobalSoftwareTest extends MarriottIntlCorpLib {
 ...
 public void Tc_MIC02(int StartRow, String EndRow, boolean nextTestJoin) throws Throwable {
	try {
			...
			int intEndRow=ReportControl.fnGetEndRowCunt(EndRow, "MIC02_MIGlobalSoftware", TestDataInsight, "MarriottIntl_Corp");
		...
		try {
			...
			Hashtable<String, String> data = TestUtil.getDataByRowNo("MIC02_MIGlobalSoftware", TestDataInsight,"MarriottIntl_Corp", intCounter);
			this.reporter.initTestCaseDescription("MIGlobalSoftware");
			...
			cmtLib.setPermissionsToDisable(data.get("Menu_Name"), data.get("Set_Permission"));
				...
				ReportStatus.fnUpdateResultStatus("MIGlobalSoftware","MIC02", ReportStatus.strMethodName, intCounter, browser);
				fnCloseTest();
			}
		} catch (Exception e) {
			e.printStackTrace();
			...
			gErrorMessage = e.getMessage();
			gTestStatus = false;
			ReportStatus.fnUpdateResultStatus("MIGlobalSoftware","MIC02", ReportStatus.strMethodName, 1, browser);
