...
public class PID09_AddToCompareListFromPersonalProductsListTest extends ActionEngine{
	...
	// #############################################################################################################
		...
		public void Tc_PID09(int StartRow, String EndRow, boolean nextTestJoin)	throws Throwable {
			...
			int intEndRow = ReportControl.fnGetEndRowCunt(EndRow, "PID09_AddToCompareListFromPersonalProductsListTest",TestDataInsight, "Product_Detail");
			for (int intCounter = intStartRow; intCounter <= intEndRow; intCounter++) {
				try {
				fnOpenTest();
				ReportStatus.fnDefaultReportStatus();
				ReportControl.intRowCount = intCounter;
				Hashtable<String, String> data = TestUtil.getDataByRowNo("PID09_AddToCompareListFromPersonalProductsListTest",TestDataInsight, "Product_Detail", intCounter);
				TestEngineWeb.reporter.initTestCaseDescription("AddToCompareListFromPersonalProductsListTest");
		...
		catch (Exception e) 
				{
					...
					ReportStatus.fnUpdateResultStatus("SearchIncludingProduct", "PID09", ReportStatus.strMethodName, intCounter, browser);
					fnCloseTest();
				}
			} 
			catch (Exception e)
			{
				...
				gErrorMessage = e.getMessage();
				gTestStatus = false;
				ReportStatus.fnUpdateResultStatus("SearchIncludingProduct", "PID09", ReportStatus.strMethodName, 1, browser);
